...
def _run_skim_gpt(job_dir: str, params: HypothesisEvalJobParams) -> dict:
    ...
    Args:
        ...
        params: Request params with structure like:
        {
            "model": "r1",
    ...
    data = params['data']
    is_km = 'KM_hypothesis' in params
    is_skim = 'SKIM_hypotheses' in params
    is_dch = bool(params.get('is_dch', False))
    ...
    # Inject dynamic values from params
    config['KM_hypothesis'] = params.get('KM_hypothesis', '')
    config['SKIM_hypotheses'] = params.get('SKIM_hypotheses', {})
    config['GLOBAL_SETTINGS']['MODEL'] = params.get('model', 'o3-mini')
    config['GLOBAL_SETTINGS']['TOP_N_ARTICLES_MOST_CITED'] = params.get('top_n_articles_most_cited', 50)
    config['GLOBAL_SETTINGS']['TOP_N_ARTICLES_MOST_RECENT'] = params.get('top_n_articles_most_recent', 50)
    config['GLOBAL_SETTINGS']['POST_N'] = params.get('post_n', 5)
...
def _write_token_to_file(htcondor_token: str, token_dir: str) -> str:
    ...
    # HTCondor tokens need to be in the format of a JWT with header.params.signature
...
def _get_config_template():
    ...
    return {
        ...
        "KM_hypothesis": "",  # Will be injected from params
        "SKIM_hypotheses": {},
        "Evaluate_single_abstract": False,
        "GLOBAL_SETTINGS": {
            "A_TERM": "",
            "A_TERM_SUFFIX": "",
            "TOP_N_ARTICLES_MOST_CITED": 50,  # Will be injected from params
            "TOP_N_ARTICLES_MOST_RECENT": 50,  # Will be injected from params
            "POST_N": 5,
            "MIN_WORD_COUNT": 98,
            "MODEL": "r1",  # Will be injected from params
